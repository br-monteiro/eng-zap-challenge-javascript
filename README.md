## Desafio OLX
API de consulta de registros para ZAP e Viva Real
...
Prevendo que um dos requisitos de uma API desse tipo é ser rápida e confiável, optei por usar um **HashMap** para o armazenamento dos registros (em memória), onde cada APIKey (`zap`, `viva-real`, … `n`) tem seu próprio mapa de registros.
...
**Foram implementados os seguintes handlers:**
 ...
 - **SetDataVivaReal**: após as validações, insere no storage os registros de acordo com as regras para APIKey viva-real
 - **SetDataZap**: após as validações, insere no storage os registros de acordo com as regras para APIKey zap
...
Esta rota recebe no corpo da requisição o objeto JSON informando qual a URL do arquivo de source.
...
Para fazer isto, basta mandar uma requisição do tipo POST para a rota **/api/v1/load** informando no corpo da requisição, qual o endereço do arquivo source. Exemplo:
...
`/api/v1/zap?filter[city]=São Paulo`

`/api/v1/zap?filter[city]=São Paulo&filter[businessType]=sale`

`/api/v1/zap?filter[city]=São Paulo&filter[businessType]=sale&filter[neighborhood]=Santana`
...
https://eng-zap-challenge-javascript.herokuapp.com/api/v1/viva-real

https://eng-zap-challenge-javascript.herokuapp.com/api/v1/zap
...
Mesmo se tratando de uma API de teste e sabendo que não será usada em produção, é importante deixar mapeado alguns pontos de melhoria:
 ...
 - Implementar persistência usando SGDB
...
```bash
$ docker run --rm --name eng-zap-challenge-javascript_app -v "$PWD":/opt/app eng-zap-challenge-javascript_app:latest npm test
```
#### conclusão
